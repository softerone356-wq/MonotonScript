...
if HAS_FILE then
    ...
    pcall(function()
        ...
        if isfile(KEY_FILE) then
            ...
            if readfile(KEY_FILE) == PERMANENT_KEY then
                HasLicense = true
            end
...
Label(
    Home,
    "ESP • Sky • Particles • Movement • Models",
    25
)
...
Slider(
    ...
    CFG.ESPMaxDistance,
    function(v)
        CFG.ESPMaxDistance = math.floor(v)
    end
)
...
Slider(
    ...
    CFG.ChinaHatHeight,
    function(v)
        CFG.ChinaHatHeight = v
    end
)

Label(
    Models,
    "Wide conical visual hat",
    26
)
...
Label(
    Info,
    "ESP: Box / Fill / Outline / Health / Corner / Skeleton",
    38
)

Label(
    Info,
    "Movement: Noclip / Fly / Air Jump / Speed",
    38
)
...
local function ClearAura()
    ...
    for _,obj in ipairs(AuraParts) do
...
local function UpdateAura()
    ...
    for _,petal in ipairs(AuraParts) do
...
local function UpdateESP()
    ...
    for _,player in ipairs(
        ...
        -- Health bar is positioned to the left.
        data.HealthBack.CFrame =
            root.CFrame *
            CFrame.new(
                -2.25,
                0,
...
local GreenCorrection =
    Lighting:FindFirstChild(
        "MonotonGreenCorrection"
    )

if GreenCorrection then
    GreenCorrection:Destroy()
end
...
GreenCorrection.Name =
    "MonotonGreenCorrection"

GreenCorrection.Enabled =
    false

GreenCorrection.Parent =
    Lighting
...
local function MakeGreenSky()
    ...
    GreenCorrection.Saturation =
        .35

    GreenCorrection.Contrast =
        .08

    GreenCorrection.Brightness =
        .04
...
print(
    "[MonotonVisuals] V1.2 loaded"
)

print(
    "[MonotonVisuals] ESP / Fly / Noclip / China Hat ready"
)
